  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-[#08090d] text-slate-100">
      ...
      <motion.aside 
        ...
        className="border-r border-white/10 bg-slate-950/50 backdrop-blur-2xl flex flex-col relative"
      >
        <div className="p-8 space-y-6 overflow-y-auto flex-1">
          <div className="space-y-2">
            ...
            <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-pink-500 to-rose-500 w-0 transition-all duration-500 shadow-[0_0_12px_rgba(244,63,94,0.6)]" />
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between group cursor-pointer">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  Essencial
                  <ChevronDown className="w-4 h-4 text-slate-400 group-hover:text-pink-400 transition-colors" />
                </h3>
              </div>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all text-left group ${
                      activeLessonId === lesson.id 
                        ? 'bg-white/[0.08] backdrop-blur-xl border border-pink-400/40 shadow-[0_0_25px_rgba(244,63,94,0.2)]' 
                        : 'hover:bg-white/[0.04] border border-transparent'
                    }`}
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all ${
                      activeLessonId === lesson.id 
                        ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-[0_0_15px_rgba(244,63,94,0.4)] border border-white/20' 
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-bold truncate ${
                        activeLessonId === lesson.id ? 'text-pink-300' : 'text-slate-200'
                      }`}>
                        {lesson.title}
                      </p>
      ...
      <div className="flex-1 overflow-y-auto flex flex-col">
        ...
        <header className="p-8 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest">
            <span className="hover:text-pink-400 cursor-pointer transition-colors">Início</span>
            <ChevronRight className="w-3 h-3" />
            <span className="hover:text-pink-400 cursor-pointer transition-colors">Essencial</span>
            <ChevronRight className="w-3 h-3" />
        ...
        <div className="px-8 pb-16 max-w-6xl w-full mx-auto space-y-8">
          {/* Video Player Area */}
          <div className="relative aspect-video w-full glass-panel rounded-[3rem] overflow-hidden shadow-[0_25px_60px_rgba(0,0,0,0.6)] group border border-white/20">
            <img 
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-8 bg-gradient-to-t from-slate-950 via-slate-950/60 to-transparent p-12">
              <div className="text-center space-y-4 max-w-2xl">
                <p className="text-pink-400 font-bold text-sm uppercase tracking-[0.3em]">Módulo 0{activeLesson.order}</p>
              ...
              <button className="px-12 py-5 bg-gradient-to-r from-pink-500 to-rose-500 hover:opacity-90 text-white rounded-full font-bold text-lg shadow-[0_0_25px_rgba(244,63,94,0.4)] border border-white/20 transition-all hover:scale-105 active:scale-95 flex items-center gap-3">
          ...
          <div className="flex items-center justify-between py-6 border-b border-white/10">
            ...
            <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-8 py-3 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-500 text-white text-sm font-bold shadow-[0_0_20px_rgba(244,63,94,0.3)] border border-white/20 hover:opacity-90 transition-all disabled:opacity-30"
              >
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 pb-4 text-sm font-bold transition-all relative ${
                    activeTab === tab.id ? 'text-pink-400' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  ...
                  {activeTab === tab.id && (
                    <motion.div 
                      layoutId="activeTab" 
                      className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-pink-500 to-rose-500 rounded-t-full shadow-[0_0_12px_rgba(244,63,94,0.6)]"
                    />
                  )}
                </button>
              ))}
                  ...
                  {activeTab === 'materials' && (
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        ...
                        <div key={i} className="flex items-center justify-between p-4 glass-panel rounded-2xl group hover:border-pink-500/40 transition-all cursor-pointer">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-slate-300 group-hover:bg-pink-500/20 group-hover:text-pink-300 transition-all">
                          ...
                          <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-pink-400 transition-all" />
                        </div>
                      ))}
                    </div>
                  )}
                    ...
                    <div className="space-y-6">
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 space-y-4">
                          ...
                          />
                          <button className="px-8 py-3 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-sm font-bold rounded-full hover:opacity-90 transition-all shadow-[0_0_20px_rgba(244,63,94,0.3)] border border-white/20">
                            Enviar Comentário
                          </button>
                        </div>
                      </div>
