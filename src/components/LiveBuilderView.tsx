...
export function LiveBuilderView() {
  ...
  return (
    <main className="mx-auto max-w-[1400px] space-y-6 px-4 py-7 sm:px-6 lg:px-10">
      ...
      <nav className="grid grid-cols-4 gap-1 rounded-2xl border border-white/10 bg-white/[.025] p-2 sm:gap-2">
      ...
      <section className="min-h-[540px] rounded-[2.25rem] border border-white/10 bg-[#0b0908] p-5 sm:p-8">
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: .2 }}>
            ...
            {step === 3 && <div><StepTitle eyebrow="Etapa 4" title="Revise antes de ir para a Shopee" description="Confira sua seleção. Ao continuar, o anúncio do produto será aberto diretamente na Shopee."/><div className="mt-8 grid gap-5 lg:grid-cols-3"><SummaryCard icon={PackageCheck} label="Produto selecionado" title={selectedProduct.name} detail={`${selectedProduct.price} · Comissão ${selectedProduct.commission}`} image={selectedProduct.image}/><SummaryCard icon={Video} label="Vídeo selecionado" title={selectedVideo.productName} detail="Criativo escolhido para apoiar sua preparação" image={selectedVideo.thumbnail}/><SummaryCard icon={Flame} label="Apresentação" title="Preparação concluída" detail={`${minutes} min planejados para o produto`}/></div><div className="mt-6 rounded-2xl border border-white/10 bg-white/[.025] p-5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-slate-500">Pontos de apoio</p><p className="mt-2 text-sm leading-relaxed text-white">{talkingPoints}</p>{personalNote && <><p className="mt-4 text-[10px] font-black uppercase tracking-[.16em] text-slate-500">Observação pessoal</p><p className="mt-2 text-sm leading-relaxed text-white">{personalNote}</p></>}</div></div>}
          </motion.div>
        </AnimatePresence>
      </section>
